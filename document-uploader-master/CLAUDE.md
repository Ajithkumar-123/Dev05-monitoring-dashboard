<!-- ai-dlc-bootstrap:aidlc-start version=v0.1.8 source=core-workflow.md@v0.1.8 -->
...
## ai-dlc-bootstrap layer

This repository uses [ai-dlc-bootstrap](https://github.com/opus2/ai-dlc-bootstrap) to layer specialist agents and lazy guidance over AI-DLC.

- **Specialist agents:** `.claude/agents/aidlc-*` — invoke via the Task tool with `subagent_type="aidlc-<name>"`.
- **Guidance Skills:** `.claude/skills/aidlc-*` — invoke via `/aidlc-<topic>`. For specialist agents, prefer Pattern B Read-on-demand (see `aidlc-authoring`); avoid `skills:` frontmatter unless the agent always needs the body from token-zero.
- **Authoring conventions:** `.claude/skills/aidlc-authoring/SKILL.md` — read before authoring or reviewing aidlc-* artifacts.

After installing or updating ai-dlc-bootstrap, restart Claude Code (or run `/agents`) so new agents and Skills are picked up.

<!-- ai-dlc-bootstrap:aidlc-end -->
